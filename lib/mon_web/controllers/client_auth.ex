defmodule MonWeb.ClientAuth do
  import Plug.Conn
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "client_remember_me"
  ...
  def log_out_client(conn) do
    ...
    client_token && Accounts.delete_session_token(client_token)
  ...
  def fetch_current_client(conn, _opts) do
    ...
    client = client_token && Accounts.get_client_by_session_token(client_token)
    assign(conn, :current_client, client)
  end

  defp ensure_client_token(conn) do
    if client_token = get_session(conn, :client_token) do
      {client_token, conn}
    else
      conn = fetch_cookies(conn, signed: [@remember_me_cookie])

      if client_token = conn.cookies[@remember_me_cookie] do
        {client_token, put_session(conn, :client_token, client_token)}
      else
        {nil, conn}
      end
    end
  ...
  defp maybe_store_return_to(%{method: "GET"} = conn) do
    ...
    put_session(conn, :client_return_to, return_to)
  end
