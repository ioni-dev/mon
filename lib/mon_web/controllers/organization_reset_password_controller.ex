defmodule MonWeb.OrganizationResetPasswordController do
  ...
  def create(conn, %{"organization" => %{"email" => email}}) do
    if organization = Accounts.get_organization_by_email(email) do
      Accounts.deliver_organization_reset_password_instructions(
        organization,
  ...
  def edit(conn, _params) do
    render(conn, "edit.html", changeset: Accounts.change_organization_password(conn.assigns.organization))
  end
  ...
  def update(conn, %{"organization" => organization_params}) do
    case Accounts.reset_organization_password(conn.assigns.organization, organization_params) do
      {:ok, _} ->
        conn
  ...
  defp get_organization_by_reset_password_token(conn, _opts) do
    ...
    if organization = Accounts.get_organization_by_reset_password_token(token) do
